...
Net::Net(const vector<unsigned> &topology) {
	...
	for (unsigned layerNum = 0; layerNum < numLayers; layerNum++) {
		Layer *novaCamada = new Layer();

		if (novaCamada == NULL)
			cout << "Problema de instanciação de nova camada.." << endl;

		m_layers.push_back(novaCamada);
		...
		for (unsigned neuronNum = 0; neuronNum <= topology[layerNum]; neuronNum++) {
			Neuron *novoNeuron = new Neuron(numOutputs, neuronNum);

			if (novoNeuron == NULL)
				cout << "Novo neuron não alocado.." << endl;

			m_layers.back()->push_back(novoNeuron);
...
void Net::backPropagation (const vector<double> &targetVals) {
	...
	for (unsigned layerNum = m_layers.size() - 1; layerNum > 0; layerNum--) {
		...
		for (unsigned n = 0; n < layer.size() - 1; n++) {
			...
			//cout << "Chagamo aqui" << layer.size() << endl;
		}
	}
}
